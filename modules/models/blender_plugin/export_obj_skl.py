...
def write_file(filepath, objects, scene,
    ...
    EXPORT_GLOBAL_QUATERNION = EXPORT_GLOBAL_MATRIX.to_quaternion()
    ...
    sklfilepath = os.path.splitext(filepath)[0] + ".skl"
    with ProgressReportSubstep(progress, 2, "OBJ Export path: %r" % filepath, "OBJ Export Finished") as subprogress1:
        with open(filepath, "w", encoding="utf8", newline="\n") as obj_file:
            ...
            # Write Header
            head_fw('# Blender v%s OBJ File: %r\n' % (bpy.app.version_string, os.path.basename(bpy.data.filepath)))
            head_fw('# www.blender.org\n')
            ...
            def add_armature(armature_object):
                ...
                def add_bone(bone):
                    ...
                    parent = 0
                    if bone.parent:
                        parent = bone_ids[bone.parent] + 1
                    pos = armature_object.location + bone.matrix_local.translation
                    pos = EXPORT_GLOBAL_MATRIX * pos
                    head_fw('#> b %d %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %s\n' % (
                        parent,
                        pos.x, pos.y, pos.z,
            ...
            for i, ob_main in enumerate(objects):
                ...
                for ob, ob_mat in obs:
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        ...
                        for v in me_verts:
                            fw('v %.6f %.6f %.6f\n' % v.co[:])
                            vertex_weights = {vertGroupNames[g.group]: g.weight for g in v.groups}
                            bones = {}
                            for modifier in ob_armature_modifiers:
                                armature = modifier.object.data
                                for bone in armature.bones:
                                    if modifier.use_vertex_groups and bone.name in vertex_weights:
                                        bones[bone] = bones.get(bone, 0) + vertex_weights[bone.name]
                                    if modifier.use_bone_envelopes:
                                        bones[bone] = bones.get(bone, 0) + bone.evaluate_envelope(v.co)

                            bones = {k: v for k, v in bones.items() if v != 0}
                            if len(bones) > 0:
                                for bone, weight in bones.items():
                                    (armature_id, bone_id) = flat_ids[bone]
                                    fw('#> vw %d %d %.6f\n' % (armature_id + 1, bone_id + 1, weight))
            ...
            obj_file.write(header_output)
            obj_file.write(body_output)
